...
不同於傳統廣告攔截器，SkipAds **不阻止廣告播放**——廣告仍會完整播出（只是加速），曝光不受影響。同時我們會自動記錄廣告資訊（廣告主、文案等），讓有興趣的用戶可以事後回顧，不錯過真正有用的廣告。

## 功能

- **自動跳過廣告** — 偵測跳過按鈕並自動點擊（預設關閉，需手動開啟）
...
- **統計資料** — 記錄廣告、推薦商品與首頁廣告的統計，包含次數、節省時間與最近紀錄（最多各 50 筆）
...
- ⚠️ 自動點擊略過按鈕的行為可能被 YouTube 誤判為 adblock 插件。**預設關閉此功能**，若你願意承擔被偵測的風險，可在設定中手動開啟。
...
**v2.7** — 自動點擊開關 & 反偵測 & 首頁廣告統計
- 新增「自動點擊略過按鈕」開關：**預設關閉**，避免被 YouTube 誤判為 adblock；用戶可手動開啟
- 新增「首頁廣告」獨立統計區塊：紀錄首頁出現的廣告文案與廣告主，可獨立清除
- 新增設定頁面選項，用戶可自行切換
- 更新繁體中文與 English 翻譯
